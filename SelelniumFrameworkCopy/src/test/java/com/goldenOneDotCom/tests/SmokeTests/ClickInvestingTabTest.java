...
public class ClickInvestingTabTest {
    ...
    public void setUp(@Optional("chrome") String browser, ITestContext context){
        ...
        test = extentReport.createTest("Validate Investing Tab behavior after click","Validate that after clicking the investing tab all elements are displayed");
    ...
    private void verifyInvestingTabLinksDisplayed() {
        try{
            ...
            WebElement subNavMenu = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class=\"sub-nav open\"]")));
            if(subNavMenu.isDisplayed()){
                //Locating every link element
                WebElement makeInvestmentPlanLink = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[contains(text(),'Make an Investment Plan')]"))); 
                WebElement selfGuidedInvestingLink = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[contains(text(),'Self-Guided Investing')]")));
                WebElement findInvestmentAdvisorLink = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[contains(text(),'Find an Investment Advisor')]")));
                WebElement insuraceProtectionOptionsLink = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a[contains(text(),'Insurance & Protection Options')]")));
            ...
            //a[contains(text(),'Make an Investment Plan') and @data-analytics-link-type='internal']
    ...
    private void clickInvestingTab(){
        try{
            WebDriverWait wait = new WebDriverWait(driver,Duration.ofSeconds(TestConstantsTest.DEFAULT_WAIT_TIME_SECONDS));
            WebElement investingTabLink = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[contains(text(),'Investing') and @role=\"button\"]")));
